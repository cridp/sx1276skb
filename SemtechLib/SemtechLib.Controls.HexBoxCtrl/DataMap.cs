...
{
	internal sealed class DataMap : ICollection, IEnumerable
	{
		internal sealed class Enumerator : IEnumerator, IDisposable
		{
		...
		private DataBlock GetLastBlock()
		{
			...
			for (var dataBlock = FirstBlock; dataBlock != null; dataBlock = dataBlock.NextBlock)
			{
				result = dataBlock;
			}
			return result;
